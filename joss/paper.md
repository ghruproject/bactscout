...
BactScout is a Python-based pipeline for rapid, standardized quality assessment and taxonomic profiling of raw read data from sequencing cultured bacterial isolates. It integrates tools like Fastp for short-read quality control, Nanoq for long-read quality control, Sylph for species-level taxonomic profiling, and StringMLST for multi-locus sequence typing into a single, reproducible workflow. Designed for scientists working with large-scale bacterial genome data, BactScout provides interpretable quality metrics across read quality, species purity, GC content, long-read yield and N50, and strain typing. These outputs support downstream applications such as genome assembly, resistance prediction, mapping, variant calling, and phylogenetic analysis. The pipeline features a modular and extensible architecture with configurable quality thresholds, parallel sample processing, and detailed per-sample and batch-level reporting.
...
Runtime behavior is configured through YAML files defining database paths, defaults, and quality thresholds. A consistent two-tier WARN/FAIL model allows users to adjust sensitivity to their context (e.g., stricter for clinical pipelines, relaxed for research). The short-read and long-read workflows are parallel command paths with separate output schemas, allowing long-read QC to be added without changing the established Illumina interface. Each run generates per-sample directories containing structured CSV summaries, tool logs, and reports. The summaries include stable, well-documented field names for easy integration into LIMS or dashboards. `summary` commands aggregate per-sample outputs into batch-level reports (`final_summary.csv` for short reads and `final_summary_long.csv` for long reads). The typical runtime for a 2 million read short-read sample is 30-60 seconds with less than 8 GB RAM on one thread, scaling linearly with sample number.
