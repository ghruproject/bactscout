...
def parse_kat_hist(hist_file: str, config: Dict) -> Dict[str, float]:
    ...
    try:
        ...
        # Error peak analysis
        error_cov_cutoff = config.get("kat", {}).get("thresholds", {}).get("error_cov_cutoff", 4)
        error_instances = sum(cov * count for cov, count in rows if cov <= error_cov_cutoff)
        error_peak_prop = error_instances / total_instances if total_instances > 0 else 0.0
        ...
        logger.info("Parsed histogram: %d distinct, %d instances", total_distinct, total_instances)
        return metrics
...
def parse_kat_gcp(gcp_file: str, config: Dict) -> Dict[str, float]:
    ...
    try:
        ...
        multi_modal_threshold = config.get("kat", {}).get("thresholds", {}).get(
            "gcp_multi_modal_bin_prop", 0.1
        )
        
        high_bins = {(gc, cov): count for (gc, cov), count in bins.items()
        ...
        main_peak_cov = config.get("kat", {}).get("thresholds", {}).get("main_cov_low", 10)
        lowcov_threshold = main_peak_cov * 0.2
        
        lowcov_extreme_gc = sum(
            count for (gc, cov), count in bins.items()
            if cov < lowcov_threshold and (gc < 0.25 or gc > 0.75)
        )
        
        metrics["kat_gcp_lowcov_gc_prop"] = lowcov_extreme_gc / total_instances if total_instances > 0 else 0.0
        
        logger.info("Parsed GCP: %d bins, multi-modal=%d", len(bins), metrics["kat_gcp_multi_modal"])
        return metrics
...
def run_kat_analysis(
    ...
    if kat_config.get("run", {}).get("hist", True):
        hist_file = run_kat_hist(kat_binary, read1_file, read2_file, output_dir, k, threads)
        if hist_file:
            results.update(parse_kat_hist(hist_file, config))
    
    # Run GC×coverage analysis
    if kat_config.get("run", {}).get("gcp", True):
        gcp_file = run_kat_gcp(kat_binary, read1_file, read2_file, output_dir, k, threads)
        if gcp_file:
